...
const Event = () => {
  ...
  const checkEventConflicts = (newEvent) => {
    return events.some((existingEvent) => {
      return (
        (newEvent.startTime >= existingEvent.start &&
          newEvent.startTime < existingEvent.end) ||
        (newEvent.endTime > existingEvent.start &&
          newEvent.endTime <= existingEvent.end) ||
        (newEvent.startTime <= existingEvent.start &&
          newEvent.endTime >= existingEvent.end)
      );
    });
  ...
  const handleUpdateEvent = async (e) => {
    ...
    if (currentEvent.eventID) {
      const conflictCheckEvents = events.filter(
      ...
      const hasConflicts = tempEvents.some(
        (existingEvent, index) =>
          index < conflictCheckEvents.length &&
          ((currentEvent.startTime >= existingEvent.start &&
            currentEvent.startTime < existingEvent.end) ||
            (currentEvent.endTime > existingEvent.start &&
              currentEvent.endTime <= existingEvent.end) ||
            (currentEvent.startTime <= existingEvent.start &&
              currentEvent.endTime >= existingEvent.end))
      );
      ...
      try {
        await axios.put(
          `${BASE_URL}/updateEvent/${currentEvent.eventID}`,
          currentEvent
        );
        fetchEvents();
        resetForm();
    ...
    <div>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {currentEvent.eventID ? "Edit Event" : "Create Event"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="col-md-12">
            ...
            <form
              onSubmit={
                currentEvent.eventID ? handleUpdateEvent : handleCreateEvent
              }
            >
                ...
                <input
                  ...
                  value={
                    currentEvent.startTime
                      ? moment(currentEvent.startTime).format(
                          "YYYY-MM-DDTHH:mm"
                        )
                      : ""
                  }
                ...
                <input
                  ...
                  value={
                    currentEvent.endTime
                      ? moment(currentEvent.endTime).format("YYYY-MM-DDTHH:mm")
                      : ""
                  }
                  onChange={(e) => {
                    setCurrentEvent((prev) => ({
